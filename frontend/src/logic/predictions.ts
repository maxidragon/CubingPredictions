...
export const addPodiumPrediction = async (
  ...
  if (
    checkPodiumPrediction(firstPlaceWcaId, secondPlaceWcaId, thirdPlaceWcaId)
  ) {
    const prediction = {
      competitionId,
      competitionName,
...
export const checkPodiumPrediction = (
  firstPlaceWcaId: string,
  secondPlaceWcaId: string,
  thirdPlaceWcaId: string,
) => {
  if (
    ...
    return !(
      firstPlaceWcaId === secondPlaceWcaId ||
      firstPlaceWcaId === thirdPlaceWcaId ||
      secondPlaceWcaId === thirdPlaceWcaId
    );
  }
};

export const getYourPrediction = async (
  competitionId: string,
  eventId: string,
) => {
  try {
    const response = await backendRequest(
      `predictions/podium/my/${competitionId}/${eventId}`,
      "GET",
      true,
    );
...
export const getAllUserPredictions = async (userId: number) => {
  try {
    const response = await backendRequest(
      `predictions/user/podium/${userId}`,
      "GET",
      true,
    );
    return await response.json();
